...
def main():
    ...
    console.print("[bold blue]🤖 Downloading spaCy models...[/bold blue]")
    from core.download_models import download_all_models
    try:
        download_all_models()
    except Exception as e:
        console.print(f"[yellow]Warning: Failed to download spaCy models: {str(e)}[/yellow]")
        console.print("[yellow]You can try downloading them later using: python -m core.download_models[/yellow]")
    
    # Installation complete panel
    panel_text = (
        t("Installation completed") + "\n\n" +
        t("Now you can start the application by running:") + "\n" +
        "[bold]python start.py[/bold]\n\n" +
        t("This will:") + "\n" +
        "1. " + t("Check and download required models") + "\n" +
        "2. " + t("Verify all dependencies") + "\n" +
        "3. " + t("Start the Streamlit server") + "\n\n" +
    ...
    troubleshoot_text = (
        t("If you encounter any issues:") + "\n" +
        "1. " + t("Check your network connection") + "\n" +
        "2. " + t("Re-run the installer: [bold]python install.py[/bold]") + "\n" +
        "3. " + t("Try downloading models manually: [bold]python -m core.download_models[/bold]")
    )
    console.print(Panel(troubleshoot_text, style="yellow"))

    if inquirer.confirm(
        message=t("Would you like to start the application now?"),
        default=True
    ).execute():
